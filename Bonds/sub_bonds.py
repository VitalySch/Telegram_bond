...
class SUB_BONDS:
  def __init__(self, prof, year, bond_sort):
    ...
    link = "https://smart-lab.ru/q/subfed/"
    ...
    bonds_SUB = bonds[1: -1]
    for key in range(len(bonds_SUB)):
      
      name = bonds_SUB[key][2]
      if len(bonds_SUB[key][10]) > 2:  
        cost = float(bonds_SUB[key][9]) * 10
        
      cupon = float(bonds_SUB[key][11])
      cupon_in_year = int(float(bonds_SUB[key][12]))
      date = bonds_SUB[key][4].split('-')
